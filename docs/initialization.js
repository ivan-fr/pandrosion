...
export function exactDouble(x){
 if(!(x>0&&Number.isFinite(x)))throw Error('A strictly positive finite number is required.');
...
export function residualInterval(p,X,s){
 if(!Number.isSafeInteger(p)||p<1||p>1_000_000)throw Error('An integer degree from 1 to 1,000,000 is required.');
...
function bandSpec(name){const b=initializationBands[name];if(!b)throw Error('Unknown band.');return b;}
...
export function findInitialState(p,X,band='compact'){
 bandSpec(band);
 if(!Number.isSafeInteger(p)||p<3||p>1_000_000||!(X>0&&Number.isFinite(X)))throw Error('Choose an integer p from 3 to 1,000,000 and a positive finite X.');
 ...
 for(let i=0;i<80;i++){
  ...
  else throw Error('Comparison cannot be decided at the available certification precision.');
 }
 throw Error('No certified starting point found at the available precision.');
}
export function initializeCalibrated(p,X,band='compact'){
 ...
 if(compareDyadic(bound.hi,bound.e,high.lo,high.e)>0||compareDyadic(represented.hi,represented.e,low.lo,low.e)>0)throw Error('Calibration error exceeds the certified bound.');
...
export function iterationDecision(p,X,current,next,band='compact'){
 ...
 if(compareDyadic(before.lo,before.e,lo.lo,lo.e)<0||compareDyadic(before.hi,before.e,hi.hi,hi.e)>0)throw Error('The current starting point is outside the checked numerical band. Initialize again.');
 if(compareDyadic(after.lo,after.e,lo.lo,lo.e)<0||compareDyadic(after.hi,after.e,hi.hi,hi.e)>0)throw Error(`The step leaves the checked numerical band [${bspec.guardLower}, ${bspec.upper}]. Starting point preserved.`);
