...
/-! Exact gate and minimal diagonal degree for p=32767; generated rational proof inputs. -/
noncomputable section
namespace LeanMath.Papers.V14Minimal32767
open Polynomial
...
def rho : ℝ := (((6108953263116889/576460752303423488) : ℚ) : ℝ)

def A0 : ℝ[X] := ofCoeffs [1]

def cert0 : Certificate A0 ((32767:ℚ):ℝ) rho 0 :=
  certificate [] [0,-32767] 32767 (6108953263116889/576460752303423488) 2 0
    (by norm_num) (by norm_num) (by norm_num) (by norm_num)
    (by norm_num [mass]) (by norm_num [mass]) (by norm_num [mass])
    (by decide +kernel) (by decide +kernel)

theorem diagonal0 (y : ℝ) (hy : |y|≤rho) :
    correction (1/32767) 0 (unchi y)=A0.eval y/A0.eval (-y) := by
  have hm : y ∈ Set.Ioo (-1) 1 := abs_lt.mp (hy.trans_lt cert0.hr1)
  have hz := (cert0.hpos (-y) (by simpa using hy)).ne'
  have hden := mul_ne_zero
    (poly_pos (-(1/32767)) (by norm_num) 0 (unchi y) (unchi_pos y hm).le).ne'
    (poly_pos (1/32767) (by norm_num) 0 1 (by norm_num)).ne'
  unfold correction
...
theorem excludes0 : ¬Meets 32767 rho 0 := by
...
def A1 : ℝ[X] := ofCoeffs [1,(1/32767)]

def cert1 : Certificate A1 ((32767:ℚ):ℝ) rho 2 :=
  certificate [(1/32767)] [0,(-1073676290/32767),0,(1/32767)] 32767 (6108953263116889/576460752303423488) (2147352576/1073676289) 2
    (by norm_num) (by norm_num) (by norm_num) (by norm_num)
    (by norm_num [mass]) (by norm_num [mass]) (by norm_num [mass])
    (by decide +kernel) (by decide +kernel)

theorem diagonal1 (y : ℝ) (hy : |y|≤rho) :
    correction (1/32767) 1 (unchi y)=A1.eval y/A1.eval (-y) := by
  have hm : y ∈ Set.Ioo (-1) 1 := abs_lt.mp (hy.trans_lt cert1.hr1)
  have hz := (cert1.hpos (-y) (by simpa using hy)).ne'
  have hden := mul_ne_zero
    (poly_pos (-(1/32767)) (by norm_num) 1 (unchi y) (unchi_pos y hm).le).ne'
    (poly_pos (1/32767) (by norm_num) 1 1 (by norm_num)).ne'
  unfold correction
  apply (div_eq_div_iff hden hz).mpr
  have h1 : 1-y≠0 := by linarith [hm.2]
...
theorem excludes1 : ¬Meets 32767 rho 1 := by
...
def A2 : ℝ[X] := ofCoeffs [1,(1/32767),(-357892096/1073676289)]

def cert2 : Certificate A2 ((32767:ℚ):ℝ) rho 4 :=
  certificate [(1/32767),(-357892096/1073676289)] [0,(-1789460482/32767),0,(896607268444372993/35181150961663),0,(-128086752379273216/35181150961663)] 32767 (6108953263116889/576460752303423488) (1024694019749969920/1152780773560811521) 4
    (by norm_num) (by norm_num) (by norm_num) (by norm_num)
    (by norm_num [mass]) (by norm_num [mass]) (by norm_num [mass])
    (by decide +kernel) (by decide +kernel)

theorem diagonal2 (y : ℝ) (hy : |y|≤rho) :
    correction (1/32767) 2 (unchi y)=A2.eval y/A2.eval (-y) := by
  have hm : y ∈ Set.Ioo (-1) 1 := abs_lt.mp (hy.trans_lt cert2.hr1)
  have hz := (cert2.hpos (-y) (by simpa using hy)).ne'
  have hden := mul_ne_zero
    (poly_pos (-(1/32767)) (by norm_num) 2 (unchi y) (unchi_pos y hm).le).ne'
    (poly_pos (1/32767) (by norm_num) 2 1 (by norm_num)).ne'
  unfold correction
  apply (div_eq_div_iff hden hz).mpr
  have h1 : 1-y≠0 := by linarith [hm.2]
...
theorem uniform : Meets 32767 rho 2 := by
...
theorem minimal_degree : Meets 32767 rho 2 ∧ ∀ j : ℕ, j<2 → ¬Meets 32767 rho j := by
  refine ⟨uniform,?_⟩
  intro j hj
  interval_cases j
  · exact excludes0
...
theorem gate_contains_entry (X : ℝ) (hX : 0<X) (hwork : |Real.log X|≤1000*Real.log 2) :
    |chi (LeanMath.Papers.V14Entry.residual 32767 X 1)|≤rho := by
  apply (LeanMath.Papers.V14HalleyThresholds.entry_radius 32767 X (by norm_num) hX hwork).trans
  norm_num [LeanMath.Papers.V14HalleyThresholds.radius,rho]

end LeanMath.Papers.V14Minimal32767
