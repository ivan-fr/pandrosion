...
namespace LeanMath.Papers.Cayley.Quintic
open Real Filter Set
open scoped Topology

def gap (p y : ℝ) := logCoord (P₃ p y)-logCoord y/p
def factor (p y : ℝ) := 2*a₃ p*(2/p-3*p+a₃ p*y^2)/(p*((1-(P₃ p y)^2)*(1-y^2)))
def error (p e : ℝ) := e+log (C₅ p (exp (-p*e)))
def coefficient (p : ℝ) := (p^2-1)*(3*p^2-2)/240

theorem polynomial_mem (p y : ℝ) (hp : 1 < p) (hy : y ∈ Ioo (-1) 1) :
    P₃ p y ∈ Ioo (-1) 1 := by
  simpa [truncation] using truncation_mem 1 p y hp hy

theorem polynomial_derivative (p y : ℝ) : HasDerivAt (P₃ p) (a₁ p+3*a₃ p*y^2) y := by
  convert! ((hasDerivAt_id y).const_mul (a₁ p)).add
    (((hasDerivAt_id y).pow 3).const_mul (a₃ p)) using 1 <;>
    simp [id_eq,mul_comm,mul_left_comm,mul_assoc]

theorem hasDerivAt_gap (p y : ℝ) (hp : 1 < p) (hy : y ∈ Ioo (-1) 1) :
    HasDerivAt (gap p) (y^4*factor p y) y := by
...
theorem gap_order (p : ℝ) (hp : 1 < p) :
    Tendsto (fun y : ℝ => gap p y/y^5) (𝓝[≠] 0) (𝓝 (-2*(p^2-1)*(3*p^2-2)/(15*p^5))) := by
  have hzero : gap p 0=0 := by simp [gap,logCoord,P₃]
  have hc := (hasDerivAt_gap p 0 hp (by norm_num)).continuousAt
  have hd : ∀ᶠ y in 𝓝[≠] (0:ℝ), HasDerivAt (gap p) (y^4*factor p y) y := by
  ...
  have h := LocalOrder.from_derivative 4 (gap p) (factor p) hzero hc (factor_continuous p hp) hd
  have hp0 : p ≠ 0 := by linarith
  have he : factor p 0 / ((4:ℝ)+1) = -2*(p^2-1)*(3*p^2-2)/(15*p^5) := by
...
/-- Exact order 5, in the root-log error, for every real p>1. -/
theorem exact_order (p : ℝ) (hp : 1 < p) :
    Tendsto (fun e : ℝ => error p e/e^5) (𝓝[≠] 0) (𝓝 (coefficient p)) := by
  have hp0 : p ≠ 0 := by linarith
  ...
  have hprod := ((gap_order p hp).comp hcoord).mul (hslope.pow 5)
  have hconst : (-2*(p^2-1)*(3*p^2-2)/(15*p^5))*(-p/2)^5=coefficient p := by
    unfold coefficient
    field_simp
    <;> ring
  rw [hconst] at hprod
...
end LeanMath.Papers.Cayley.Quintic
