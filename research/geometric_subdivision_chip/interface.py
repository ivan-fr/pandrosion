...
def run(p,X,bits=20):
 ...
 path=ROOT/'results/interface_raw'/f'p{p}_X{X}';path.mkdir(parents=True,exist_ok=True)
 ctrl=['.control','set numdgt=17','set wr_singlescale','set wr_vecnames','tran 1n 2.51u 0 1n','wrdata trace.txt v(held) v(n_load) v(dac)','quit','.endc','.end']
 ...
 output=code*lsb*meta['scale'];reference=X**(1/p)
