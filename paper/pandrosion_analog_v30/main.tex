...
\subsection{Replacing the nested chain by geometric binary powering}
The fast construction stores a value $v$ at $R(v)=(W,H(1-v))$ on the right rail. Prepare $U_\times=(W+H,H)$ and the fixed transfer line $U_\times B$. Copy $R(a)$ to the top rail by a parallel to this line. The intersection is $Z(a)=(W+Ha,H)$. Join $U_\times$ to $R(b)$; its parallel through $Z(a)$ meets the right rail at $R(ab)$. Thus the same gadget performs a square when $a=b$ and a multiplication otherwise.

To see the product, the join has slope $b$ and the horizontal displacement from $Z(a)$ to the right rail is $-Ha$. The output ordinate is therefore $H-Hab$. No evaluation of $a b$ is needed to construct the intersection. For $p=13=(1101)_2$, reuse the gadget along
\[
 s\longmapsto s^2\longmapsto s^3\longmapsto s^6\longmapsto s^{12}\longmapsto s^{13}.
\]
The last point is the same $E=R(s^p)$ used in \eqref{eq:geometry}. Only the power-producing construction changes; $M$, the transfer $FD=AE$, the support $AD$ and the final report remain as above. Five multiplications replace twelve in this example. These are multiplication counts, not total line counts: copies, joins and parallels must also be counted.

The existing \texttt{RectangleFastPower} Lean module certifies the copy and product intersections and the full geometric recursion through \texttt{geometricMul\_R}, \texttt{geometricPower\_eq} and \texttt{geometric\_readout}. These geometric certificates are separate from the six centered circuit identities listed later.
...
\section*{Geometric fast exponentiation feeding AD}
\begin{figure}[H]\centering
\includegraphics[width=\linewidth]{figures/fast_geometry.pdf}
\caption{The replacement for the nested proportional triangles. (a) Two parallel constructions give $R(ab)$; setting $a=b$ gives a square. The illustrative factors are $a=0.64$, $b=0.42$. (b) The binary output $E$ enters the unchanged AD construction, shown for $p=13$, $X=W=2$, $H=4$, $s=0.82$. The support is extended through $T$ and the horizontal gives $P^+$. (c) Five invocations of the product gadget give $E=R(s^{13})$; numbers below the boxes are the encoded values, not ordinates. Every intersection is computed and checked by the figure-generation script.}
\end{figure}
In this example $s^{13}\approx0.07578445$ and the geometric report gives $s^+\approx0.91852928$, agreeing with \eqref{eq:ad}. The example illustrates the geometric mechanism, not the circuit's digitally prepared initial condition.

The top unit point used here matches the Lean certificate. The gallery uses $U_\times=(3W,H)$, with the corresponding horizontal scale $2W$ instead of $H$; both choices realize the same product. They coincide for the illustrated $W=2$, $H=4$. The theorem concerns ideal intersections. Near-coincident points in a finite-precision drawing do not acquire a conditioning guarantee from the binary stage count.

The circuit uses the same exponent schedule but replaces these line intersections with the centered polynomial stages derived next. A geometric diagram and an electrical circuit are distinct realizations of the exact arithmetic.
\clearpage
...
\section*{Electrical schematic of the simulated core}
\begin{figure}[H]\centering
\includegraphics[width=\linewidth]{figures/analog_circuit.pdf}
\caption{Electrical topology of the precision-mode SPICE model. Repeated node labels denote the same net. (a) Candidate filtering, sample storage, calibrated voltage buffer and transfer to state. (b) Explicit state capacitance, leakage resistance, net leakage/compensation current, injected disturbances and reset switch. (c) The loaded cell used for the read buffer and every centered power stage. Diamonds are behavioral voltage sources, not transistor-level amplifiers. The orange dashed arrow indicates voltage sensing, not a conductive path.}
\end{figure}
The three switch models use $R_{\mathrm{on}}=230~\Omega$ and $R_{\mathrm{off}}=10^{12}~\Omega$. The sample window is 350--750~$\mu$s and the transfer window 770--1170~$\mu$s of each 1200~$\mu$s period. Reset grounds the state during the initial 10~$\mu$s. The readout occurs 15~$\mu$s after transfer ends. These control sources are prescribed waveforms, not an implemented timing circuit.

For the read cell, $f_j$ senses \texttt{state} and includes read offset, noise and fitted compensation. Each power cell uses \eqref{eq:square} or \eqref{eq:multiply} with the modeled gain/offset and fitted trim terms. The residual and denominator are behavioral sources feeding \eqref{eq:centered}; \texttt{Bcandidate} clips that corrected target before its 10~$\Omega$/100 nF output filter. They are specified by equations rather than a transistor schematic.

The ADC and final digital decode are outside this electrical drawing because they are postprocessed transfer models. Power rails, transistor internals and the physical implementation of trims are not supplied by the existing model. The drawing therefore documents exactly the level simulated; it is not a fabrication-ready circuit.
\clearpage
