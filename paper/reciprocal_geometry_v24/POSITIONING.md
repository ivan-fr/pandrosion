# V24: the decentered arc under specified equivalences

This supplement answers the last reading report's question without proposing another solver. All parameters below satisfy **real p > 2** unless an integer restriction is stated. These are written arguments with exact symbolic checks; they are **outside the Lean audit of Theorem 4.3**.
...
Run `python3 paper/reciprocal_geometry_v24/src/positioning/verify.py` from the repository root. It checks the elimination, discriminants, primitive-coefficient identities, exact residual coefficients, fifth-order consistency and scaling law. Integer square-freeness examples supplement the universal written argument; they do not replace it. The existing uniform verifier independently derives all three fifth-order coefficients.
