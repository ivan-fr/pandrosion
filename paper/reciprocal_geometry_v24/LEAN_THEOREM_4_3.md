...
The V24 fixed-circle theorem is formalized for **every real `p > 2`**. The degree is not restricted to natural numbers. The three modules introduced in V23 compile with the repository's pinned Lean/mathlib toolchain; the audit covers all **69 declarations**, including definitions of the inverse and the iteration.
