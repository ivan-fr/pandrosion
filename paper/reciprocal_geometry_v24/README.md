...
[Manuscript](Reciprocal_Root_Geometry_v24.pdf) · [LaTeX](main.tex) · [Changes from V23](REVISION_NOTES.md) · [Validation](VALIDATION.md) · [Literature ledger](LITERATURE_REVIEW.md)

V24 keeps the two solvers, abstract and constructions of V23. It addresses the final review question about the decentered arc's equivalence to classical methods. V23 is preserved as an archive.

## Changes from V23

- Section 2.3 separates formula identity, homographic changes and local analytic conjugacy. The [full positioning argument](POSITIONING.md) states each domain and limitation.
- For integer p≥3, the arc state map is nonrational and cannot be Möbius-conjugate to a rational solver. For every real p>2, the correction itself cannot be rationalized by input/output homographies.
- Its primitive quadratic has contact six, excluding the specified diagonal type (2,2,2) Hermite–Padé/Shafer normalization requiring eight in the same variable.
- The arc and rational Padé **are locally real-analytically conjugate** for every real p>2, by the classical Böttcher theorem. This local equivalence does not transport geometric protocols, global domains or cost.
- No method or Lean declaration is added. The exact checks and two additional primary references support a bounded comparison, not exhaustive historical priority.

## Proof and evidence map

| Claim | Evidence and exact scope |
|---|---|
| Equivalence comparisons in Section 2.3 | Written [positioning argument](POSITIONING.md), classical Böttcher theorem, 18 exact symbolic checks and 10 finite integer regression cases. Outside the Lean audit. |
...
```bash
python3 paper/reciprocal_geometry_v24/src/positioning/verify.py
python3 paper/reciprocal_geometry_v24/src/uniform/verify.py
...
All three audits allow only `propext`, `Classical.choice` and `Quot.sound`. The unchanged V23 audit checks every declaration in its three source modules, including the noncomputable inverse. These modules are part of the default Lean build; GitHub Actions also runs this audit, the positioning checks, the real-parameter verifier and the V24 PDF build. V24 outputs are in [verification](verification/); the original Lean build and audits remain in [V23 verification](../reciprocal_geometry_v23/verification/).
