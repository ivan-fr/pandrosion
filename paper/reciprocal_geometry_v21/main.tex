...
\]
Its reference point is $v=1$. At $p=3,t=0.05$, both inverse roots are positive, approximately 3.1183 and 6.3817; only the first is on this branch. Positivity alone cannot choose $G$. At a transverse endpoint the two roots merge. Whole-interval contraction remains an explicitly separate conjecture in the archived study; the theorem above is local. Section~\ref{sec:safeguard} gives a global algorithm by geometrically testing a candidate and falling back to AD.
\clearpage
...
Fixed-circle inverse & 5 & $2p+2$ & 0 & 0 & Local\\
Guarded circle--AD & 5 & \multicolumn{3}{c}{Section~\ref{sec:safeguard}} & Global\\\bottomrule
\end{tabularx}
\end{table}
...
\section{A geometric safeguard makes the circle algorithm global}\label{sec:safeguard}
The unguarded circle has a bounded real domain. A geometric acceptance test can nevertheless place it inside a globally convergent algorithm. Cache $E=R(s^p)$ and $T=R(t)$, $t=Xs^p$. If $T=B$, stop. Attempt the prescribed descending circle branch; a missing transverse intersection or singular readout triggers centered AD. For a positive candidate $c$, construct $u=Xc^p$ and accept exactly when
\begin{equation}\label{eq:gate}
 \begin{cases}
 t\le u^2\quad\hbox{and}\quad tu^2\le1,&t\le1,\\
 u^2\le t\quad\hbox{and}\quad1\le tu^2,&t\ge1.
 \end{cases}
\end{equation}
This uses three ordered comparisons on the same rail after forming $R(u^2)$ and $R(tu^2)$. Otherwise use AD, then refresh the cache. On acceptance, the trial's power and residual become the next cache.

\begin{theorem}[Globally convergent circle--AD controller]\label{thm:guard}
For every integer $p\ge3$, $X>0$ and $s_0>0$, this exact geometric protocol converges to $X^{-1/p}$. On the prepared chart regular at the root it eventually uses the circle update, unless it has already reached the root, and inherits local order five.
\end{theorem}
\begin{proof}
For $t,u>0$, taking logarithms shows that~\eqref{eq:gate} is equivalent to $|\log u|\le|\log t|/2$. Thus every accepted step halves the nonnegative error $\epsilon(s)=|\log(Xs^p)|$. AD lies between the state and the root, so fallback never increases this error. The errors decrease to a limit $\ell\ge0$.

If $\ell>0$, eventually $\epsilon<3\ell/2$. A halving step would give $\epsilon^+<3\ell/4$, contradicting the lower bound $\ell$. Hence every sufficiently late step would be AD. Its global convergence implies $\ell=0$, a contradiction. Therefore $\epsilon\to0$, and $s=a\exp(\log t/p)\to a$.

Near $a$, the fifth-order circle result gives $\epsilon(c)/\epsilon(s)\to0$, so the gate accepts throughout a punctured neighborhood. Use the compact preparation for $p\ne4$ and the separate degree-four preparation from Appendix~\ref{app:prep}. At $v=1$, $v\ne\rho$ and $b(v)\ne0$: for the compact chart,
\[
 b(1)=\frac{48p(p+4)(p+1)}{(p-1)\Lambda_p}>0,
\]
and the degree-four value is nonzero by substitution. The discriminant is $36p^2>0$. Continuity therefore supplies the required regular neighborhood.
\end{proof}
The global argument does not assume contraction on the entire circle branch. The Lean development proves the gate equivalence, the switching theorem, its circle specialization and the incidence-based controller. Eventual selection and local-order transfer are formalized with analytic premises; the concrete inverse-function assembly remains the written local proof above.

\subsection{A control made only of joins}
Given $R(a),R(b)$ with $b>0$, first test $R(b)=B$. If equal, reuse $R(a)$. Otherwise draw $CR(b)$ to $V=\hub{1/b}$. Project $R(a)$ through $\hub{1/2}$ to $L(a/2)$; joining this point to $V$ gives $R(ab/2)$. Two further joins, through $\hub{1/2}$ and then $\hub{1/4}$, give $L(ab/4)$ and $R(ab)$. This is a five-join product with finite intersections for $b\ne1$.

A fixed positive scale $d$ costs two joins: project left through $\hub{d/[2(1+d)]}$ and right through $\hub{1/[2(1+d)]}$. Both centers are finite. Binary products and this scaling construct the residual without a root oracle, including at $s=1$.
\clearpage

\subsection{The price of a checked step}
Let $m=\lfloor\log_2p\rfloor+\operatorname{popcount}(p)-1$, the binary multiplication count. With the current power and residual cached, a circle trial costs six joins: two to form $R(kt)$, one to save $L(\rho s)$, and the final three circle/readout joins. Computing the trial residual costs $5m+2$; the two control products cost ten more.

\begin{table}[H]\centering\small
\caption{Safeguarded protocol upper bounds, including the next cache. Unit products may use fewer joins. Prepared objects, incidence tests and order comparisons are separate from these trace columns.}
...
Accepted circle step with control & $5m+18$ & 0 & 0\\
Full trial rejected, AD and cache refresh & $10m+22$ & 2 & 1\\\bottomrule
\end{tabularx}
\end{table}
A completed gate makes three ordered comparisons and $m+2$ product identity tests; initial and fallback cache construction each add $m$ identity tests. Root stopping and circle branch/incidence tests are also explicit decisions. A missing crossing permits an earlier, cheaper fallback. The trace inventory is a written protocol count checked by an instrumented intersection implementation.

\fig{circle_safeguard}{1}{Left: the five joins compute a product from rail points, with $a=1.4$ and $b=0.6$. Right: an illustrative trial with $t=2,u=1.1$ passes the three order comparisons. The ticks and labels annotate one oriented rail; they are not additional construction joins. The controller uses products and order, not the unknown root.}

For $p=3$, the initial cache uses at most 12 joins; an accepted step uses at most \textbf{28 joins} and no new arc. A full rejection costs $42J+2P_{\parallel}+C_{\mathrm{arc}}$, or 53 expanded traces. At $p=10^6$, $m=25$ and acceptance costs at most 143 joins. These are upper bounds, not optimality claims. In particular, the eight-line local construction is not a global eight-line algorithm.

The implementation checks 144 geometric trajectories at 180 digits, including degrees through $10^6$, plus rejected trials, exact unit inputs and singular charts. All tested trajectories reach $|\log t|<10^{-45}$ within 18 steps. This supplements the exact convergence theorem. Rail points can be arbitrarily distant, and the product center diverges as $b\to1$ despite the exact-unit shortcut. Global exact existence therefore does not imply uniform drawing size or floating-point conditioning.
\clearpage
...
The \href{https://github.com/ivan-fr/pandrosion/blob/main/paper/reciprocal_geometry_v21/README.md}{V21 repository guide} links sources, proofs and verification commands. The \href{https://ivan-fr.github.io/pandrosion/}{interactive gallery} shows planar constructions and their optional spherical display.

Independent intersections, symbolic identities and scalar comparisons check 162 circle configurations at 250 digits, 108 arc configurations at 160 digits, and 144 safeguarded trajectories at 180 digits. The safeguard has a separate axiom audit. Frozen V20 and analog V30 packages retain detailed antecedents and circuit evidence.
