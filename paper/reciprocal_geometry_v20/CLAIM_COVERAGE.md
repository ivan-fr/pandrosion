...
| Neutral AD reciprocal closure adds nothing | Written algebra in CONTEXT_SUPPLEMENT.tex | Not included in geometry audit; no global theorem transferred to reset-state map |
| Earlier center-defect variants | Retained V18 source `sections/05_bilateral_acceleration.tex` and companion; outside the seven protocols in V20 | Not part of fixed-circle cost theorem or the 149-declaration audit |
...
| Historical novelty, maximal order, whole real basin | No theorem asserted | No proof established |
...
## V20 additions and corrections

| Statement | Evidence | Boundary |
|---|---|---|
| Correct projective factorization | Existing Lean `RectangleProjective.numerator_difference`; `src/revision/verify.py` | V19 typesetting/algebra error corrected; Lean theorem unchanged |
| Uniform rational circle preparation for every integer p ≥ 3 | Written substitution certificate; exact SymPy identity in `src/revision/verify.py` | Not in the 149-declaration Lean audit; no uniform compactness claim |
| Finite construction theorem exclusions | Explicit hypotheses in Theorem 5.1 and geometric argument | Prepared objects, selected branch, noncoincident joins; not global physical cost |
| Dyadic renormalization | Scalar conjugacy and written bounds in Section 5.2 | Controls the initial center near the limit; does not remove all residual exceptions |
| Full-branch strict logarithmic error decrease | Conjecture and 12,024 high-precision samples, `src/revision/checks.json` | No general proof or Lipschitz contraction theorem |
| All seven browser constructions | `docs/geometry.js`, `scripts/test_geometry.mjs`, `scripts/test_browser.mjs` at repository root | 576 geometric cases plus UI checks; ordinary doubles, not formal rendering verification |

V19 source snapshots remain archived with their original validation records; the repository erratum identifies the corrected identity. The updated bibliography supplies context without claiming historical priority.
